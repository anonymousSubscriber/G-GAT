...
def build_degree_matrix(adj):
    adj = sp.coo_matrix(adj)
    row_sum = np.array(adj.sum(1))
    row_sum=(row_sum==0)*1+row_sum
    d_inv_sqrt = row_sum.flatten()
    d_inv_sqrt[np.isinf(d_inv_sqrt)] = 0.
    degree_matirx = (d_inv_sqrt)
    return torch.tensor(degree_matirx).reshape(-1,1).long()
...
def position_map(degree_matrix,c=2):

    mean_degree=torch.sum(degree_matrix)/degree_matrix.shape[0]*c
    degree_matrix=torch.where(degree_matrix>mean_degree,
        torch.ones_like(degree_matrix)*mean_degree,degree_matrix)
...
def adj2degree(adj):
    res=torch.sum(to_one(adj),dim=-1).long()
    return res

def get_distance_matrix(adj,max_distance=10):
    MAX_NUM=max_distance*2
    mat=torch.eye(adj.shape[0])
    ones_mat=torch.ones_like(adj)
    res=ones_mat*MAX_NUM
    for distance in range(max_distance):
        mask=torch.where(mat>0,ones_mat*distance,ones_mat*MAX_NUM)
        res=torch.min(mask,res)
        mat=torch.mm(adj,mat)
    return res.long()
